              ...
              <h1 class="text-3xl font-bold mb-6 text-center">12종 게임 시리즈</h1>
                  <div class="relative">
                   <div class="absolute w-full h-full pointer-events-none" >
                        ${emojis.map((emoji,i) => `
                             <span style="animation-delay: ${Math.random()}s; top: ${Math.random() * 100}%; left: ${Math.random() * 100}%; font-size: ${Math.random() * 2 + 3}rem;" class="absolute animate-floatEmoji">${emoji}</span>
                          `).join("")}
                    </div>
                      <div class="grid grid-cols-3 gap-4 mb-8">
                        ${gameData.games.map(game => `
                          <div class="p-4 flex justify-center items-center">
                              <button data-gameid="${game.id}"  class="w-24 h-24  bg-cover bg-center rounded-full shadow-md hover:shadow-lg transform hover:scale-105 transition-transform duration-300" style="background-image: url('${game.thumbnail}');">
                                </button>
                           </div>
                       `).join('')}
                       </div>
